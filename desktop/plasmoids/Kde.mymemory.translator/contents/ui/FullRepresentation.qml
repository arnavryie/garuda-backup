...
Item {
    ...
    Timer {
        id: lanVerificationAvailableTimer
        repeat: false
        interval: 800   // 800ms debounce real

        onTriggered: {
            console.log("API de detección ejecutándose")

            Api.detectLanguage(txt.text, function(error, detectedLang) {

                if (error || detectedLang === "unknown")
                    return;
                ...
                if (detectedLang === targetLang) {
                    ...
                    sourceLan.currentIndex = sourceLan.indexOfValue(sourceLang);
                    targetLan.currentIndex = targetLan.indexOfValue(targetLang);

                } else if (detectedLang !== sourceLang) {

                    sourceLang = detectedLang;
                    sourceLan.currentIndex = sourceLan.indexOfValue(detectedLang);
                }
            });
        }
    }
    ...
    Column {
        ...
        Lib.Card {
            ...
            TextField {
                ...
                onTextChanged: {
                    if (text.length > 25) {
                        lanVerificationAvailableTimer.restart()
                    }
                }
                onAccepted: {
                    textTranslate = ""
            ...
            Rectangle {
                ...
                height: width
                radius: height/2
                anchors.bottom: translateButton.top
                anchors.bottomMargin: 8
                anchors.right: parent.right
                anchors.rightMargin: - 8
                color: Kirigami.Theme.textColor
                ...
                MouseArea {
                    ...
                    onClicked: {
                        txt.paste()
                        //txt.text = Qt.application.clipboard
                    }
                }
            }
        }
        ...
        Kirigami.Icon {
            source: isPinned ? "window-unpin" : "window-pin"
            color: Kirigami.Theme.textColor
            width: 22
            height: 22
            MouseArea {
